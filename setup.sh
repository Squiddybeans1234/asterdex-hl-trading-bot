...
# One-click setup for Aster & HL Trading Bot (Linux/macOS)
...
ensure_repo() {
  ...
  local REPO_URL="https://github.com/discountry/aster-hl-trading-bot.git"
  local TAR_URL="https://github.com/discountry/aster-hl-trading-bot/archive/refs/heads/main.tar.gz"
  local TARGET_DIR="${ASTER_HL_DIR:-aster-hl-trading-bot}"
  ...
  echo "Fetching Aster & HL Trading Bot sources..."
  if command -v git >/dev/null 2>&1; then
    git clone --depth=1 "$REPO_URL" "$TARGET_DIR"
  else
    echo "ℹ git not found; downloading tarball..."
    curl -fsSL "$TAR_URL" -o /tmp/aster-hl-trading-bot.tar.gz
    mkdir -p "$TARGET_DIR"
    tar -xzf /tmp/aster-hl-trading-bot.tar.gz --strip-components=1 -C "$TARGET_DIR"
    rm -f /tmp/aster-hl-trading-bot.tar.gz
  fi
  PROJECT_DIR="$(cd "$TARGET_DIR" && pwd)"
}
