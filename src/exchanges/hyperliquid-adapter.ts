...
import { HyperliquidGateway } from "./client";
...
export class HyperliquidExchangeAdapter implements ExchangeAdapter {
    ...
    private readonly gateway: HyperliquidGateway;
    ...
    constructor(credentials: HyperliquidCredentials = {}) {
        const walletAddress = credentials.walletAddress ?? process.env.HYPERLIQUID_WALLET_ADDRESS;
        const privateKey = credentials.privateKey ?? process.env.HYPERLIQUID_PRIVATE_KEY;

        if (!walletAddress || !privateKey) {
            throw new Error("Missing HYPERLIQUID_WALLET_ADDRESS or HYPERLIQUID_PRIVATE_KEY environment variables");
        }

        this.gateway = new HyperliquidGateway({
            walletAddress,
            privateKey,
        });
    ...
    private safeInvoke<T extends (...args: any[]) => void>(context: string, cb: T): T {
        const wrapped = ((...args: any[]) => {
            try {
                ...
                console.error(`[HyperliquidExchangeAdapter] ${context} handler failed: ${extractMessage(error)}`);
            }
        }) as T;
        return wrapped;
    }
    ...
    private handleInitError(context: string, error: unknown): void {
        ...
        console.error(`[HyperliquidExchangeAdapter] ${context} failed`, error);
    }
    ...
    async cancelOrder(params: { symbol: string; orderId: number | string }): Promise<void> {
        ...
        await this.gateway.cancelOrder({ symbol: params.symbol, orderId: params.orderId });
    }
